...
(defun eval-A-times-u (src dst begin end length)
  (loop with %src0 of-type f64.2 = (make-f64.2 (aref src 0) (aref src 0))
	for i from begin below end by 2
	do (let* ((%eAt  (eval-A (make-f64.2 (+ i 0) (+ i 1)) (make-f64.2 0 0)))
		  (%sum  (f64.2/ %src0 %eAt))
	     ...
	     (setf (f64.2-ref dst i) %sum))))

(defun eval-At-times-u (src dst begin end length)
  (loop with %src0 of-type f64.2 = (make-f64.2 (aref src 0) (aref src 0))
	for i from begin below end by 2
        do (let* ((%eA   (eval-A (make-f64.2 0 0) (make-f64.2 (+ i 0) (+ i 1))))
                  (%sum  (f64.2/ %src0 %eA))
...
;;   (let ((u (make-array (+ n 1) :element-type 'd+))
;;         (v (make-array (+ n 1) :element-type 'd+))
;;         (tmp (make-array (+ n 1) :element-type 'd+)))
...
(declaim (ftype (function (int31) d+) spectralnorm))
(defun spectralnorm (n)
  (let ((u (make-array (+ n 1) :element-type 'd+ :initial-element 1.0d0))
        (v (make-array (+ n 1) :element-type 'd+))
        (tmp (make-array (+ n 1) :element-type 'd+)))
    (declare (type d+array u v tmp))
    (loop repeat 10 do
